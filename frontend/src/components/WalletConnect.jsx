...
export default function WalletConnect() {
  ...
  // ✅ Гидратация + диагностика
  useEffect(() => {
    setMounted(true);
    
    // 🐛 Диагностика: проверяем доступность кошельков
    ...
    // ⚠️ Предупреждение, если ни один кошелёк не обнаружен
    if (!hasInjected && connectors.every(c => !c.ready)) {
      setBrowserWarning('⚠️ Не обнаружено расширений кошельков. Установите MetaMask или используйте поддерживаемый браузер.');
    }
  }, [connectors, isPending, error]);
  ...
  if (!mounted) {
    return <button disabled style={styles.btnDisabled}>⏳ Загрузка...</button>;
  }

  // ✅ Если подключено: адрес + кнопка GROK
  ...
  return (
    <>
      <button onClick={() => setShowModal(true)} style={styles.connectBtn}>
      ...
      {showModal && (
        <div style={styles.overlay} onClick={() => setShowModal(false)}>
          <div style={styles.modal} onClick={(e) => e.stopPropagation()}>
            ...
            {/* ⚠️ Предупреждение, если кошельки не обнаружены */}
            {browserWarning && (
              <div style={styles.warningBox}>
                <p style={styles.warningText}>{browserWarning}</p>
                <a 
            ...
            <div style={styles.connectorsGrid}>
              {connectors.map((conn) => {
                ...
                const reason = !conn.ready 
                  ? (conn.name === 'MetaMask' ? 'Расширение не найдено' : 'Недоступен') 
                  : '';
...
const styles = {
  ...
  warningBox: { background: 'rgba(239,68,68,0.15)', borderRadius: '8px', padding: '0.75rem', marginBottom: '1rem', textAlign: 'center' },
  warningText: { color: '#fca5a5', fontSize: '0.85rem', margin: '0 0 0.5rem 0' },
  installLink: { color: '#60a5fa', textDecoration: 'none', fontWeight: '600', fontSize: '0.9rem' },
