...
// 🎨 ТЕМЫ ДОСОК
const BOARD_THEMES = {
  classic: { name: '🏛️ Классика', light: '#eeeed2', dark: '#769656', highlight: 'rgba(255,255,0,0.4)', validMove: 'rgba(20,85,30,0.5)', boardStyle: {}, pieceStyle: {} },
  neon: { name: '💜 Неон', light: '#1a1a2e', dark: '#16213e', highlight: 'rgba(236,72,153,0.5)', validMove: 'rgba(59,130,246,0.6)', boardStyle: { boxShadow: '0 0 20px rgba(59,130,246,0.3)' }, pieceStyle: { filter: 'drop-shadow(0 0 2px rgba(255,255,255,0.5))' } },
  forest: { name: '🌲 Лес', light: '#c8b59a', dark: '#5d7a4f', highlight: 'rgba(251,191,36,0.4)', validMove: 'rgba(34,197,94,0.5)', boardStyle: { borderRadius: '8px' }, pieceStyle: {} },
  ocean: { name: '🌊 Океан', light: '#a8d8ea', dark: '#2a6f97', highlight: 'rgba(255,215,0,0.4)', validMove: 'rgba(6,182,212,0.5)', boardStyle: { borderRadius: '12px', boxShadow: '0 4px 20px rgba(6,182,212,0.2)' }, pieceStyle: {} },
  sunset: { name: '🌅 Закат', light: '#ffecd2', dark: '#fcb69f', highlight: 'rgba(245,158,11,0.4)', validMove: 'rgba(239,68,68,0.5)', boardStyle: { borderRadius: '16px' }, pieceStyle: {} },
  wood3d: { name: '🪵 3D Дерево', light: '#e8c49a', dark: '#8b6f47', highlight: 'rgba(255,215,0,0.5)', validMove: 'rgba(34,197,94,0.6)', boardStyle: { borderRadius: '4px', boxShadow: '0 10px 30px rgba(0,0,0,0.4), inset 0 2px 4px rgba(255,255,255,0.3), inset 0 -2px 4px rgba(0,0,0,0.2)', backgroundImage: 'linear-gradient(135deg, rgba(255,255,255,0.1) 0%, transparent 50%, rgba(0,0,0,0.1) 100%)', transform: 'perspective(800px) rotateX(2deg)', transformStyle: 'preserve-3d', transition: 'transform 0.3s ease' }, pieceStyle: { filter: 'drop-shadow(2px 4px 3px rgba(0,0,0,0.4))', transform: 'translateZ(10px)' } }
}

const COUNTRIES = [
  { code: 'RU', name: '🇷🇺 Россия' }, { code: 'UA', name: '🇺🇦 Украина' }, { code: 'BY', name: '🇧🇾 Беларусь' }, { code: 'KZ', name: '🇰🇿 Казахстан' },
  { code: 'US', name: '🇺🇸 США' }, { code: 'GB', name: '🇬🇧 Великобритания' }, { code: 'DE', name: '🇩🇪 Германия' }, { code: 'FR', name: '🇫🇷 Франция' },
  { code: 'ES', name: '🇪🇸 Испания' }, { code: 'IT', name: '🇮🇹 Италия' }, { code: 'CN', name: '🇨🇳 Китай' }, { code: 'JP', name: '🇯🇵 Япония' },
...
const getTimeLabel = (mins) => { const opts = [{v:5,l:'5 мин'}, {v:15,l:'15 мин'}, {v:30,l:'30 мин'}, {v:60,l:'1 ч'}, {v:1440,l:'24 ч'}]; return opts.find(o => o.v === mins)?.l || `${mins} мин` }
...
function App() {
  ...
  const [playerProfile, setPlayerProfile] = useState(() => { if (typeof window !== 'undefined' && address) { const saved = localStorage.getItem(`chess4crypto_profile_${address}`); return saved ? JSON.parse(saved) : { nickname: '', country: 'RU', avatar: '', socialLink: '', bio: '' } } return { nickname: '', country: 'RU', avatar: '', socialLink: '', bio: '' } })
  const [isEditingProfile, setIsEditingProfile] = useState(false)
  const [profileForm, setProfileForm] = useState(playerProfile)
  const [avatarPreview, setAvatarPreview] = useState('')
  const [gamesList, setGamesList] = useState([])

  const customSquareStyles = useMemo(() => { const theme = BOARD_THEMES[boardTheme], styles = {}; if (selectedSquare) styles[selectedSquare] = { backgroundColor: theme.highlight }; possibleMoves.forEach(sq => { styles[sq] = { backgroundColor: theme.validMove, backgroundImage: `radial-gradient(circle, rgba(255,255,255,0.9) 25%, transparent 25%)`, backgroundSize: '14px 14px', backgroundPosition: 'center' } }); return styles }, [selectedSquare, possibleMoves, boardTheme])
  const getValidMoves = useCallback((square) => gameRef.current.moves({ square, verbose: true }).map(m => m.to), [])
  const onSquareClick = useCallback((square) => { if (gameOver || lobbyMode !== 'playing') return; const piece = gameRef.current.get(square); if (piece && piece.color === (isPlayerTurn ? 'w' : 'b')) { setSelectedSquare(square); setPossibleMoves(getValidMoves(square)); return }; if (selectedSquare && possibleMoves.includes(square)) { onDrop(selectedSquare, square); setSelectedSquare(null); setPossibleMoves([]); return }; setSelectedSquare(null); setPossibleMoves([]) }, [gameOver, isPlayerTurn, selectedSquare, possibleMoves, getValidMoves, lobbyMode])
  const onDrop = useCallback((source, target) => { if (!isPlayerTurn || gameOver || moveIndex !== history.length - 1 || lobbyMode !== 'playing') return false; try { const move = gameRef.current.move({ from: source, to: target, promotion: 'q' }); if (!move) return false; const newFen = gameRef.current.fen(), san = gameRef.current.history({ verbose: true }).pop()?.san || `${source}${target}`; setHistory(prev => [...prev, newFen]); setMovesList(prev => [...prev, { san, from: source, to: target, piece: move.piece }]); setFen(newFen); setMoveIndex(prev => prev + 1); setIsPlayerTurn(false); setTimerActive('bot'); setMessage('🤖 Бот думает...'); setSelectedSquare(null); setPossibleMoves([]); if (gameRef.current.isCheckmate()) endGame('player'); else if (gameRef.current.isDraw()) endGame('draw'); else setTimeout(makeBotMove, 1000); return true } catch { return false } }, [isPlayerTurn, gameOver, moveIndex, history.length, lobbyMode])
  const makeBotMove = useCallback(() => { if (gameOver || gameRef.current.isGameOver() || lobbyMode !== 'playing') return; const moves = gameRef.current.moves(); if (!moves.length) return; const random = moves[Math.floor(Math.random() * moves.length)]; gameRef.current.move(random); const newFen = gameRef.current.fen(), san = gameRef.current.history({ verbose: true }).pop()?.san || random; setHistory(prev => [...prev, newFen]); setMovesList(prev => [...prev, { san, from: random.from, to: random.to, piece: random.piece }]); setFen(newFen); setMoveIndex(prev => prev + 1); setIsPlayerTurn(true); setTimerActive('player'); if (gameRef.current.isCheckmate()) endGame('player'); else if (gameRef.current.isDraw()) endGame('draw'); else setMessage('♟️ Ваш ход!') }, [gameOver, lobbyMode])
  const endGame = (result) => { setGameOver(true); setTimerActive(null); setSelectedSquare(null); setPossibleMoves([]); if (result === 'player') { setWinner('player'); setMessage('🏁 КОНЕЦ ИГРЫ! Вы победили!'); if (gameBalance > 0) handleClaimPrize() } else if (result === 'bot') { setWinner('bot'); setMessage('🏁 КОНЕЦ ИГРЫ! Бот победил.') } else { setWinner('draw'); setMessage('🤝 НИЧЬЯ!'); if (gameBalance > 0) handleClaimPrize() }; setLobbyMode('idle') }

  useEffect(() => { if (address) { const saved = localStorage.getItem(`chess4crypto_profile_${address}`); if (saved) { const profile = JSON.parse(saved); setPlayerProfile(profile); setProfileForm(profile); if (profile.avatar) setAvatarPreview(profile.avatar) } } }, [address])
  const saveProfile = () => { if (!address) return; const updated = { ...profileForm, avatar: avatarPreview || profileForm.avatar }; setPlayerProfile(updated); localStorage.setItem(`chess4crypto_profile_${address}`, JSON.stringify(updated)); setIsEditingProfile(false); setMessage('✅ Профиль сохранён!') }
  const handleAvatarUpload = (e) => { const file = e.target.files?.[0]; if (!file) return; if (file.size > 2 * 1024 * 1024) { setMessage('⚠️ Файл > 2MB'); return }; if (!file.type.startsWith('image/')) { setMessage('⚠️ Только изображения'); return }; const reader = new FileReader(); reader.onload = (event) => { setAvatarPreview(event.target?.result); setProfileForm(prev => ({ ...prev, avatar: event.target?.result })) }; reader.readAsDataURL(file) }

  const handleConnect = async () => { setMessage('🔄 Открываю кошелёк...'); try { const connector = isMobile() ? connectors.find(c => c.id === 'walletConnect') : (connectors.find(c => c.id === 'metaMask') || connectors.find(c => c.id === 'injected')); if (!connector) throw new Error('Кошелёк не найден'); await connect({ connector }); await new Promise(resolve => setTimeout(resolve, 1500)); if (!isConnected) throw new Error('Подключение отменено'); return true } catch (err) { setMessage('⚠️ ' + (err.shortMessage || err.message || 'Не удалось подключиться')); return false } }

  const fetchGamesList = useCallback(() => { const mockGames = [], now = Date.now(); if (address) { for (let i = 0; i < 5; i++) { mockGames.push({ id: `mock_game_${i}`, creator: `0xMock${i}...${i}${i}${i}${i}`, creatorName: `Player${i}`, stake: [1000, 5000, 10000][i % 3], timeControl: [5, 15, 30][i % 3], createdAt: now - (i + 1) * 60000, joined: false, status: 'waiting' }) } }; const saved = JSON.parse(localStorage.getItem('chess4crypto_my_games') || '[]'); saved.forEach(g => mockGames.push({ ...g, isMine: true })); setGamesList(mockGames) }, [address])
  ...
  const handleConfirmDeposit = async () => { if (!selectedGameToJoin) return; setShowDepositConfirm(false); setPendingDeposit(creatorStake); setMessage(`🔄 Вносим ${formatNumber(creatorStake)} GROK...`); try { await new Promise(resolve => setTimeout(resolve, 1500)); setGameBalance(prev => prev + creatorStake); setPendingDeposit(null); setMessage(`✅ ${formatNumber(creatorStake)} GROK внесено!`); setGamesList(prev => prev.map(g => g.id === selectedGameToJoin.id ? { ...g, joined: true, status: 'playing' } : g)); setGameId(selectedGameToJoin.id); setLobbyMode('playing'); startGame('wallet'); setSelectedGameToJoin(null) } catch (err) { console.error('Deposit error:', err); setMessage('⚠️ Ошибка'); setPendingDeposit(null); setShowDepositConfirm(true) } }
        ...
        <div style={styles.actionCards}>
          <button onClick={handleGuestLogin} style={styles.actionCard}>
            ...
            <div style={styles.actionText}>
              <strong style={{ color: '#e2e8f0', fontSize: '1rem' }}>Гостевой вход</strong>
              <span style={{ color: '#94a3b8', fontSize: '0.85rem' }}>Быстрая игра с ботом</span>
            </div>
          </button>
          <button onClick={handleConnect} style={styles.actionCard} disabled={status === 'connecting'}>
            <div style={styles.actionIcon}>{isMobile() ? '🔗' : '🦊'}</div>
            <div style={styles.actionText}>
              <strong style={{ color: '#e2e8f0', fontSize: '1rem' }}>{t('app.connectWallet')}</strong>
              <span style={{ color: '#94a3b8', fontSize: '0.85rem' }}>Игра на GROK токены</span>
            </div>
          </button>
        </div>
  ...
  if (view === 'profile') {
    ...
    return (<div style={styles.appContainer}>
      ...
      <main style={styles.profileMain}>
        <div style={styles.modernProfileCard}>
          <div style={styles.profileHeader}>
            <div style={styles.avatarWrapper}>
              {isEditingProfile ? (<label style={styles.avatarUpload}>{avatarPreview ? <img src={avatarPreview} alt="Avatar" style={styles.avatarImg} /> : <div style={styles.avatarPlaceholder}>👤</div>}<input type="file" accept="image/*" onChange={handleAvatarUpload} style={{display:'none'}} /><span style={styles.avatarEdit}>✏️</span></label>) : (<div style={styles.avatarDisplay}>{playerProfile.avatar ? <img src={playerProfile.avatar} alt="Avatar" style={styles.avatarImg} /> : <div style={styles.avatarPlaceholder}>👤</div>}</div>)}
            </div>
            <div style={styles.profileHeaderText}>
              {isEditingProfile ? (<input value={profileForm.nickname} onChange={e => setProfileForm({...profileForm, nickname: e.target.value.slice(0,20)})} placeholder="Ваш ник" style={styles.nicknameInput} />) : (<h2 style={styles.displayName}>{displayName}</h2>)}
              {isEditingProfile ? (<select value={profileForm.country} onChange={e => setProfileForm({...profileForm, country: e.target.value})} style={styles.countrySelect}>{COUNTRIES.map(c => <option key={c.code} value={c.code}>{c.name}</option>)}</select>) : (<p style={styles.countryText}>{countryName}</p>)}
            </div>
          </div>
          <div style={styles.profileActions}>
            {isEditingProfile ? (<><button onClick={saveProfile} style={styles.btnSave}>💾 Сохранить</button><button onClick={() => { setIsEditingProfile(false); setProfileForm(playerProfile); setAvatarPreview(playerProfile.avatar) }} style={styles.btnCancel}>✖️</button></>) : (<button onClick={() => { setIsEditingProfile(true); setProfileForm(playerProfile) }} style={styles.btnEdit}>✏️ Редактировать</button>)}
          </div>
          {!isEditingProfile && (playerProfile.socialLink || playerProfile.bio) && (<div style={styles.profileExtras}>{playerProfile.socialLink && <a href={formatSocialLink(playerProfile.socialLink)} target="_blank" rel="noopener noreferrer" style={styles.socialLink}>{socialIcon} {playerProfile.socialLink.replace(/^https?:\/\//, '').slice(0, 35)}{playerProfile.socialLink.length > 35 ? '...' : ''}</a>}{playerProfile.bio && <p style={styles.bio}>{playerProfile.bio}</p>}</div>)}
          {isEditingProfile && (<div style={styles.editExtras}><input value={profileForm.socialLink} onChange={e => setProfileForm({...profileForm, socialLink: e.target.value})} placeholder="https://t.me/ник или сайт" style={styles.input} /><textarea value={profileForm.bio} onChange={e => setProfileForm({...profileForm, bio: e.target.value.slice(0,150)})} placeholder="О себе..." style={{...styles.input, minHeight:'50px'}} maxLength={150} /></div>)}
        </div>
        ...
        {lobbyTab === LOBBY_TABS.available && (<div style={styles.modernLobbyCard}><h3 style={styles.lobbyTitle}>🌐 Игры других игроков</h3>{gamesList.filter(g => !g.isMine && !g.joined).length === 0 ? <p style={styles.emptyText}>Пока нет игр. Создайте свою!</p> : (<div style={styles.gamesList}>{gamesList.filter(g => !g.isMine && !g.joined).map(game => (<div key={game.id} style={styles.modernGameCard}><div style={styles.gameHeader}><span style={styles.creatorName}>{game.creatorName || 'Аноним'}</span><span style={styles.gameStake}>💰 {formatNumber(game.stake)}</span></div><div style={styles.gameDetails}><span>⏱️ {getTimeLabel(game.timeControl)}</span><span>🕐 {new Date(game.createdAt).toLocaleTimeString([], {hour:'2-digit',minute:'2-digit'})}</span></div><button onClick={() => handleJoinFromList(game)} style={styles.btnJoin}>🤝 Присоединиться</button></div>))}</div>)}</div>)}
        {lobbyTab === LOBBY_TABS.my && (<div style={styles.modernLobbyCard}><h3 style={styles.lobbyTitle}>📋 Мои игры</h3>{gamesList.filter(g => g.isMine).length === 0 ? <p style={styles.emptyText}>Вы ещё не создали игр.</p> : (<div style={styles.gamesList}>{gamesList.filter(g => g.isMine).map(game => (<div key={game.id} style={styles.modernGameCard}><div style={styles.gameHeader}><span style={styles.gameId}>🎮 {game.id.slice(-8)}</span><span style={styles.gameStatus}>{game.status === 'waiting' ? '⏳ Ожидание' : '🎮 В игре'}</span></div><div style={styles.gameDetails}><span>💰 {formatNumber(game.stake)} GROK</span><span>⏱️ {getTimeLabel(game.timeControl)}</span></div>{game.status === 'waiting' && <button onClick={() => { setInviteLink(`${getShareUrl()}?invite=${game.id}`); handleShareInvite() }} style={styles.btnShare}>📤 Пригласить</button>}</div>))}</div>)}</div>)}
        {lobbyTab === LOBBY_TABS.join && (<div style={styles.modernLobbyCard}><h3 style={styles.lobbyTitle}>🔗 По коду</h3><div style={styles.formGroup}><input value={joinCode} onChange={e => setJoinCode(e.target.value)} style={styles.input} placeholder="Код или ссылка..." /></div><button onClick={() => { if(joinCode) { setLobbyMode('join'); setMessage('Поиск...') } }} style={styles.btnPrimary}>🔍 Найти</button></div>)}
        {lobbyMode === 'join' && joinCode && lobbyTab !== LOBBY_TABS.join && (<div style={styles.modernLobbyCard}><h3 style={styles.lobbyTitle}>🔗 Приглашение</h3><p style={styles.infoText}>Код: <strong>{joinCode}</strong></p><div style={styles.inviteDetails}><div style={styles.detailRow}><span>⏱️</span><strong>{getTimeLabel(creatorTimeControl)}</strong></div><div style={styles.detailRow}><span>💰</span><strong>{formatNumber(creatorStake)} GROK</strong></div></div><button onClick={() => handleShowDepositConfirm({stake:creatorStake,timeControl:creatorTimeControl,id:joinCode})} disabled={pendingDeposit !== null} style={styles.btnPrimary}>{pendingDeposit ? '⏳...' : `💰 Внести ${formatNumber(creatorStake)}`}</button><button onClick={() => { setLobbyMode('idle'); setJoinCode(''); }} style={styles.btnSmall}>↩️</button></div>)}
        {showDepositConfirm && (<div style={styles.modalOverlay} onClick={() => setShowDepositConfirm(false)}><div style={styles.modal} onClick={e => e.stopPropagation()}><h3 style={styles.modalTitle}>💰 Подтверждение</h3><p style={styles.modalStep}>Вносите <strong>{formatNumber(creatorStake)} GROK</strong></p><p style={styles.modalStep}>Время: <strong>{getTimeLabel(creatorTimeControl)}</strong></p><div style={styles.modalActions}><button onClick={() => setShowDepositConfirm(false)} style={styles.modalCancel}>Отмена</button><button onClick={handleConfirmDeposit} style={styles.modalConfirm}>✅ Подтвердить</button></div></div></div>)}
        {lobbyMode === 'idle' && !showDepositConfirm && (<div style={styles.createSection}><h3 style={styles.lobbyTitle}>➕ Создать</h3><div style={styles.createForm}><div style={styles.formRow}><label style={styles.formRowLabel}>Ставка</label><select value={stakeAmount} onChange={e => setStakeAmount(Number(e.target.value))} style={styles.select}>{depositOptions.map(amt => <option key={amt} value={amt}>{formatNumber(amt)}</option>)}</select></div><div style={styles.formRow}><label style={styles.formRowLabel}>Время</label><select value={timeControl} onChange={e => setTimeControl(Number(e.target.value))} style={styles.select}>{timeOptionsLobby.map(o => <option key={o.value} value={o.value}>{o.label}</option>)}</select></div></div><button onClick={handleCreateGame} style={styles.btnPrimary}>🎲 Создать</button></div>)}
        {lobbyMode === 'waiting' && inviteLink && (<div style={styles.modernLobbyCard}><h3 style={styles.lobbyTitle}>⏳ Ожидание</h3><div style={styles.codeBox}>{inviteLink}</div><div style={styles.waitActions}><button onClick={() => copyToClipboard(inviteLink)} style={styles.btnSmall}>{copied ? '✅' : '📋'}</button><button onClick={handleShareInvite} style={styles.btnSmall}>📤</button><button onClick={() => { setLobbyMode('idle'); }} style={styles.btnSmallCancel}>❌</button></div></div>)}
        <div style={styles.themeSelector}><h4 style={styles.themeTitle}>🎨 Стиль доски</h4><div style={styles.themeGrid}>{Object.entries(BOARD_THEMES).map(([key, t]) => (<button key={key} onClick={() => handleThemeChange(key)} style={{...styles.themeBtn, active: boardTheme === key, theme: t}}><div style={styles.themePreview}><div style={{...styles.themeSquare, background: t.light}} /><div style={{...styles.themeSquare, background: t.dark}} /></div><span style={styles.themeName}>{t.name}</span>{boardTheme === key && <span style={styles.themeCheck}>✓</span>}</button>))}</div></div>
        <select value={i18n.language} onChange={e => i18n.changeLanguage(e.target.value)} style={styles.langSelect}><option value="ru">🇷🇺 RU</option><option value="en">🇬🇧 EN</option></select>
      </main>
    </div>)
  }
  ...
  return (<div style={styles.appContainer}>
    ...
    <main style={styles.gameMain}>
      {gameOver && winner && (<div style={styles.gameOverBanner}><div style={styles.gameOverIcon}>{winner === 'player' ? '🏆' : winner === 'bot' ? '🤖' : '🤝'}</div><div style={styles.gameOverText}>{winner === 'player' && '🎉 ПОБЕДА!'}{winner === 'bot' && '😔 Поражение'}{winner === 'draw' && '🤝 Ничья'}</div>{winner === 'player' && gameBalance > 0 && <div style={styles.prizeText}>+{formatNumber(gameBalance)} GROK</div>}<button onClick={() => { setView('profile'); setGameOver(false); setLobbyMode('idle'); }} style={styles.btnNewGame}>🏠 В лобби</button></div>)}
      <div style={styles.gameLayout}>
        <aside style={styles.sidePanel}>
          <div style={{...styles.timerCard, active: timerActive === 'player' && !gameOver}}><span style={styles.timerLabel}>👤 Вы</span><span style={styles.timerValue}>{fmtTime(playerTime)}</span></div>
          <div style={{...styles.timerCard, active: timerActive === 'bot' && !gameOver}}><span style={styles.timerLabel}>🤖 Бот</span><span style={styles.timerValue}>{fmtTime(botTime)}</span></div>
        ...
        <div style={styles.boardContainer}>
          <div style={{...styles.boardWrapper, ...theme.boardStyle}}>
            <Chessboard position={fen} onPieceDrop={onDrop} onSquareClick={onSquareClick} customSquareStyles={customSquareStyles} boardOrientation="white" customDarkSquareStyle={{ backgroundColor: theme.dark }} customLightSquareStyle={{ backgroundColor: theme.light }} customBoardStyle={{...theme.boardStyle, borderRadius: boardTheme === 'wood3d' ? '4px' : '8px'}} customPieceStyles={theme.pieceStyle} />
          </div>
        </div>
        <aside style={styles.sidePanel}>
          <div style={styles.historyPanel}>
            <div style={styles.historyHeader}><span>📜 Ходы</span><span style={styles.historyCount}>{movesList.length}</span></div>
            <div style={styles.historyList}>{movesList.map((m, i) => <span key={i} style={{...styles.moveChip, highlight: i === moveIndex - 1}}>{Math.floor(i/2)+1}.{i%2===0?'':'..'} {m.san}</span>)}{movesList.length === 0 && <span style={styles.emptyHist}>Ходов нет...</span>}</div>
          </div>
...
// 🎨 СТИЛИ (ОДИН ОБЪЕКТ - ОТКРЫВАЕТСЯ ЗДЕСЬ, ЗАКРЫВАЕТСЯ В КОНЦЕ)
const styles = {
  ...
  themeBtn: ({ active, theme }) => ({ padding: '0.6rem', borderRadius: '12px', cursor: 'pointer', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '0.4rem', background: active ? 'rgba(59, 130, 246, 0.2)' : '#0f172a', border: active ? '2px solid #3b82f6' : '2px solid #334155', transition: 'all 0.2s', position: 'relative' }),
...
} // ✅ ЗАКРЫВАЕМ ОБЪЕКТ styles ЗДЕСЬ - В САМОМ КОНЦЕ ФАЙЛА
