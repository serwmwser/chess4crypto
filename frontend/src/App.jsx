  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      if (timerActive === 'player') {
        setPlayerTime(prev => { if (prev <= 1) { endGame('bot'); return 0 } return prev - 1 })
      } else {
        setBotTime(prev => { if (prev <= 1) { endGame('player'); return 0 } return prev - 1 })
      }
    }, 1000)
...
const styles = {
  ...
  moveChip: ({ highlight, playerTurn }) => ({ background: highlight ? '#3b82f6' : playerTurn ? '#475569' : '#334155', padding: '0.15rem 0.4rem', borderRadius: '5px', fontSize: '0.75rem', fontWeight: '500' }),
