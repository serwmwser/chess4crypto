...
function App() {
  ...
  // ⏱️ ТАЙМЕР (исправлено для гостевого режима)
  ...
  const handleGuest = () => { 
    ...
    // ✅ Сбрасываем таймеры для гостевого режима
  ...
  // 🔄 Старт партии (исправлено для гостевого режима)
  const startGame = () => {
    gameRef.current.reset()
    ...
    setPossibleMoves([])
    // ✅ КЛЮЧЕВОЕ ИСПРАВЛЕНИЕ: сбрасываем таймеры на начальное значение
    setPlayerTime(timeControl * 60)
    setBotTime(timeControl * 60)
    // ✅ Запускаем таймер для игрока
  ...
  // ⏱️ ТАЙМЕР (исправлено для гостевого режима)
  useEffect(() => {
    // ✅ Если таймер не активен или игра окончена — не запускаем
  ...
  }, [timerActive, gameOver]) // ✅ Зависимости: таймер перезапускается при смене timerActive
  ...
  // ============================================================================
  return (
    <div style={{minHeight:'100vh',background:'#0f172a',color:'#f1f5f9',font# ✅ ПОЛНЫЙ КОД: `frontend/src/App.jsx` (Исправлен таймер бота в гостевом режиме)

📁 **Путь:** `C:\Users\Admin\Desktop\Chess4Crypto\chess4crypto\frontend\src\App.jsx`  
📋 **Выделите ВСЁ (`Ctrl+A`) → Удалите → Вставьте этот код → Сохраните (`Ctrl+S`)**
...
function App() {
  ...
  // ⏱️ Таймер - ИСПРАВЛЕНО: отдельные состояния для гостевого режима
  const [timeControl, setTimeControl] = useState(15)
  const [playerTime, setPlayerTime] = useState(15*60)
  const [botTime, setBotTime] = useState(15*60)
  ...
  const handleGuest = () => { 
    setMessage('👤 Гостевой режим')
    // ✅ ИСПРАВЛЕНО: Сбрасываем таймеры при старте гостевой игры
    setPlayerTime(timeControl * 60)
    setBotTime(timeControl * 60)
    startGame()
  }
  ...
  // 🔄 Старт партии - ИСПРАВЛЕНО: корректная инициализация таймеров
  const startGame = () => {
    gameRef.current.reset()
    const f = gameRef.current.fen()
    setFen(f)
    ...
    // ✅ ИСПРАВЛЕНО: Явно устанавливаем время для обоих игроков
    const gameTime = timeControl * 60
    setPlayerTime(gameTime)
    setBotTime(gameTime)
    
    // ✅ ИСПРАВЛЕНО: Запускаем таймер игрока
    setTimerActive('player')
    setView('game')
  }

  // 🏁 Конец игры - ИСПРАВЛЕНО: корректная обработка для гостевого режима
  const finishGame = (result) => {
    setTimerActive(null)
    let msg = '', change = 0
    const currentStake = pendingJoinGame?.stake || 0
    ...
    if (result === 'player') {
      ...
      change = currentStake * 2
      setWinner('player')
      // Только для подключённых пользователей обновляем баланс
      if (address) {
        saveUser({ ...userData, balance: userData.balance + change, history: [{ date: new Date().toLocaleString(), result: 'WIN', change: `+${change}` }, ...userData.history] })
      } else {
        // Для гостевого режима просто показываем сообщение
        setMessage(msg)
      }
    } else if (result === 'bot') {
      msg = '😔 Вы проиграли. Ставка ушла сопернику.'
    ...
    // Обновляем сообщение только если оно ещё не установлено
  ...
  // ⏱️ Таймер - ИСПРАВЛЕНО: корректная работа в гостевом режиме
  useEffect(() => {
    // ✅ ИСПРАВЛЕНО: Таймер работает и в гостевом режиме (без проверки address)
    if(!timerActive || gameOver) return
    ...
    return () => clearInterval(interval)
  }, [timerActive, gameOver]) // ✅ ИСПРАВЛЕНО: finishGame не в зависимостях, чтобы избежать циклов
