...
// 🎨 Темы доски
const BOARD_THEMES = {
  classic: { name: '🏛️ Классика', light: '#eeeed2', dark: '#769656' },
  wood3d: { name: '🪵 3D Дерево', light: '#e8c49a', dark: '#8b6f47' }
}
...
function App() {
  ...
  // 🔗 Подключение кошелька (с блокировкой повторов)
  const handleConnect = async () => {
    ...
    try {
      ...
      if (isConnected) { setMessage('✅ Подключено!'); setTimeout(() => startGame(), 300) }
      else setMessage('⚠️ Подключение отменено')
  ...
  // ============================================================================
  // 🎮 ИГРОВОЙ ЭКРАН (со всеми функциями)
  // ============================================================================
  return (
    <div style={{ minHeight: '100vh', background: '#0f172a', color: '#f1f5f9', fontFamily: 'system-ui, sans-serif', display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '0.8rem', boxSizing: 'border-box' }}>
      ...
      {/* 🎯 КОНТЕЙНЕР ДОСКИ */}
      <div style={{ width: boardWidth + 20, height: boardWidth + 20, background: '#1e293b', padding: '10px', borderRadius: '12px', boxShadow: '0 10px 40px rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: '1rem' }}>
        <Chessboard position={fen} onPieceDrop={onDrop} onSquareClick={onSquareClick} customSquareStyles={customSquareStyles} boardOrientation="white" boardWidth={boardWidth} customDarkSquareStyle={{ backgroundColor: theme.dark }} customLightSquareStyle={{ backgroundColor: theme.light }} />
      </div>

      {/* 🎛️ Панель управления */}
      <div style={{ display: 'flex', gap: '0.5rem', marginBottom: '1rem', flexWrap: 'wrap', justifyContent: 'center' }}>
        {/* ⏪⏩ Навигация по ходам */}
        <button onClick={() => { setSelectedSquare(null); setPossibleMoves([]); }} style={{ padding: '0.5rem 0.9rem', background: '#475569', color: '#fff', border: 'none', borderRadius: '8px', cursor: 'pointer' }}>✖️</button>
        <button onClick={handleBack} disabled={moveIndex === 0 || gameOver} style={{ padding: '0.5rem 0.9rem', background: moveIndex === 0 || gameOver ? '#334155' : '#3b82f6', color: '#fff', border: 'none', borderRadius: '8px', cursor: moveIndex === 0 || gameOver ? 'not-allowed' : 'pointer' }}>⏪</button>
        <button onClick={handleForward} disabled={moveIndex === history.length - 1 || gameOver} style={{ padding: '0.5rem 0.9rem', background: moveIndex === history.length - 1 || gameOver ? '#334155' : '#3b82f6', color: '#fff', border: 'none', borderRadius: '8px', cursor: moveIndex === history.length - 1 || gameOver ? 'not-allowed' : 'pointer' }}>⏩</button>
        
        {/* 🎨 Выбор темы */}
        {Object.entries(BOARD_THEMES).map(([key, t]) => (
          <button key={key} onClick={() => setBoardTheme(key)} style={{ padding: '0.5rem 0.9rem', background: boardTheme === key ? '#10b981' : '#334155', color: '#fff', border: 'none', borderRadius: '8px', cursor: 'pointer', fontSize: '0.9rem' }}>{t.name}</button>
        ))}
        
        {/* ⏱️ Выбор времени */}
        <select value={timeControl} onChange={e => { const v = Number(e.target.value); setTimeControl(v); if (view === 'game') { setPlayerTime(v*60); setBotTime(v*60) } }} style={{ padding: '0.5rem 0.7rem', background: '#334155', color: '#fff', border: '1px solid #475569', borderRadius: '8px', cursor: 'pointer', fontSize: '0.9rem' }}>
      ...
      <details style={{ marginTop: '1rem', padding: '0.8rem', background: '#1e293b', borderRadius: '8px', fontSize: '0.8rem', color: '#94a3b8', width: '100%', maxWidth: '400px' }}>
        ...
        <div>Timer: {timerActive || 'off'} | Turn: {isPlayerTurn ? 'player' : 'bot'}</div>
