...
export default function App() {
  ...
  const [inviteModal, setInviteModal] = useState(null); // { from, gameId }
  ...
  const startRandomMatch = () => {
    ...
    setIsSearching(true);
    socket.emit('requestMatch', { address: userAddress });
  };

  const cancelRandomMatch = () => {
    if (socket && userAddress) {
      socket.emit('cancelMatch', { address: userAddress });
      setIsSearching(false);
    }
  };
  ...
  const acceptInvite = () => {
    if (!socket || !inviteModal) return;
    setGameData({ gameId: inviteModal.gameId, role: 'black', opponent: inviteModal.from });
    setGameMode('online');
    setInviteModal(null);
  };
  ...
  const renderContent = () => {
    if (gameMode === 'bot') {
      return (
        <div style={{ padding: '2rem' }}>
          ...
          <h2 style={{ textAlign: 'center' }}>♟️ vs Бот</h2>
    ...
    if (isSearching) {
      return (
        <div style={{ padding: '3rem', textAlign: 'center' }}>
          ...
          <p style={{ color: '#94a3b8' }}>Ожидаем подключения второго игрока</p>
    ...
    switch (activeTab) {
      case 'home':
        return (
          <div style={{ padding: '2rem', textAlign: 'center' }}>
            ...
            {!isAuthorized && (
              <button onClick={() => { setIsGuest(true); setActiveTab('play'); }} style={styles.btnPrimary('#10b981')}>
                👤 Войти как гость
              </button>
            )}
          </div>
      ...
      default:
        return <div style={{ padding: '2rem', textAlign: 'center' }}><h2>🚧 Раздел в разработке</h2></div>;
    }
  };
      ...
      {inviteModal && (
        <div style={styles.overlay} onClick={() => setInviteModal(null)}>
          <div style={styles.modal} onClick={e => e.stopPropagation()}>
            <h3 style={{ color: '#f59e0b', marginBottom: '1rem' }}>📨 Приглашение в игру</h3>
            <p style={{ color: '#e2e8f0', marginBottom: '1rem' }}>
              <strong>{inviteModal.from?.slice(0,6)}...{inviteModal.from?.slice(-4)}</strong> приглашает вас!
            </p>
            <div style={{ display: 'grid', gap: '0.5rem' }}>
              <button onClick={acceptInvite} style={{ padding: '0.75rem', background: '#10b981', color: '#fff', border: 'none', borderRadius: '8px', cursor: 'pointer', fontWeight: 'bold' }}>✅ Принять и войти</button>
