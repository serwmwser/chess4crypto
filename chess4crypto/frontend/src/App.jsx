...
useEffect(()=>{const r=()=>setBs(Math.min(window.innerWidth-40,400));updateSize();window.addEventListener('resize',r);return()=>window.removeEventListener('resize',r)},[])
...
const connectWallet=async()=>{
  try{
    if(typeof window.ethereum!=='undefined'){
      await open() // Открывает модалку Web3Modal (работает на мобильных)
    }else{
      window.location.href='https://metamask.io/download/'
    }
  }catch(e){setMsg(t('noMetaMask'))}
}
...
// 💰 СОЗДАНИЕ ИГРЫ + ДЕПОЗИТ В КОНТРАКТ
const handleCreateMatch=async()=>{
  ...
  try{
    ...
    console.error(e)
    setMsg(e.message?.includes('rejected')||e.message?.includes('denied')?t('txCancelled'):t('errTx')+(e.shortMessage||''))
...
<div style={{display:'flex',alignItems:'center',gap:'0.8rem'}}>
  ...
  <div><div style={{fontWeight:'bold'}}>{profile.name||address?.slice(0,6)+'...'+address?.slice(-4)}</div><div style={{color:COLORS.textSec,fontSize:'0.9rem'}}>{t('bal')} <span style={{color:COLORS.accent}}>{userBalance.toLocaleString()} GROK</span></div></div>
</div>
...
{profileTab==='playerProfile'&&<div style={{background:COLORS.cardBg,padding:'1rem',borderRadius:'12px'}}>
  ...
  {isEditingProfile?(<div style={{display:'flex',flexDirection:'column',gap:'0.8rem'}}>
  ...
  </div>):(<div>{profile.avatar&&<img src={profile.avatar} alt="av" style={{width:80,height:80,borderRadius:'50%',objectFit:'cover',margin:'0 auto 1rem',display:'block',border:'3px solid '+COLORS.accent}}/>}
    <p><b>{t('name')}:</b> {profile.name||'—'}</p><p><b>{t('bio')}:</b> {profile.bio||'—'}</p>
    ...
    <button onClick={()=>setIsEditingProfile(true)} style={BtnStyle(COLORS.btnBlue)}>{t('playerProfile')}</button></div>)}
</div>}
...
{profileTab==='lobby'&&<div style={{background:COLORS.cardBg,padding:'1rem',borderRadius:'12px'}}>
  ...
  {availableGames.length>0?availableGames.map(g=>{const totalPot=(g.stake||0)*((g.cpaid?1:0)+(g.hpaid?1:0));return(<div key={g.id}style={{background:'rgba(0,0,0,0.2)',padding:'0.8rem',borderRadius:'8px',marginBottom:'0.8rem'}}><div style={{display:'flex',justifyContent:'space-between'}}><b>ID: ...{g.id.slice(-6)}</b><span style={{color:g.status==='playing'?COLORS.accent:COLORS.textSec}}>{g.status}</span></div><div style={{fontSize:'0.8rem',color:COLORS.textSec,margin:'0.5rem 0'}}>{t('st')}: {g.stake} | ⏱️ {g.time_limit}мин | {t('totalPot')}: {totalPot}GROK</div>{!g.hpaid&&g.creator!==address&&<button onClick={()=>{setPendingJoin({id:g.id,stake:g.stake,time:g.time_limit});handleJoinMatch()}} disabled={loadingTx} style={{...BtnStyle('#10b981',loadingTx),marginTop:'0.5rem'}}>{t('jn')} ({g.stake} GROK)</button>}</div>)}):<p style={{color:COLORS.textSec}}>{t('noG')}</p>}
</div>}
...
<button onClick={handleCreateMatch}disabled={loadingTx}style={BtnStyle('#10b981',loadingTx)}>{loadingTx?t('approveTx'):t('cr')}</button>
