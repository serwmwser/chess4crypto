...
const{data:depositReceipt,isSuccess:depositConfirmed}=useWaitForTransactionReceipt({hash:txHash})
...
// ✅ Отслеживаем подтверждение депозита
useEffect(()=>{
...
// ✅ Исправлено: безопасное копирование для мобильных
const copyToClipboard=async(text)=>{
  try{
    // Метод 1: стандартный Clipboard API
  ...
  try{
    // Метод 2: fallback для мобильных (execCommand)
...
// 💰 СОЗДАНИЕ ИГРЫ + ДЕПОЗИТ + ОЖИДАНИЕ ПОДТВЕРЖДЕНИЯ
const handleCreateMatch=async()=>{
  ...
  try{
    ...
    // ✅ Депозит с получением хэша для отслеживания
    const depositTx=await writeContractAsync({address:CHESS_CONTRACT,abi:CHESS_ABI,functionName:'deposit',args:[bytes32Id]})
    console.log('Deposit TX:',depositTx)
    setTxHash(depositTx) // ✅ Запускаем useWaitForTransactionReceipt
    
    // ✅ Сразу создаём запись в Supabase (cpaid=true)
    ...
    // ✅ Безопасное копирование ссылки
    await copyToClipboard(link)
    
    setMsg(t('confirmingTx')) // ✅ Показываем статус ожидания
