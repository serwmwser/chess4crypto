...
import{parseUnits,formatUnits,waitTransactionReceipt}from'viem'
...
const handleCreateMatch=async()=>{
  ...
  try{
    ...
    // В viem строки автоматически конвертируются в bytes32 если ABI требует
    ...
    loadActiveGames() // ✅ Обновляем табло сразу
    setProfileTab('my') // ✅ Переключаем на "Мои игры"
  }catch(e){
    console.error(e)
...
const handleClaim=(isDraw)=>{if(!gameId){setMsg('⚠️ Game not finished');return}if(!isConnected||!address){setMsg(isDraw?t('drawRefund'):t('winnerGets'));setGameState('idle');return}
...
{activeGames.map(g=>{const totalPot=(g.stake||0)*((g.cPaid?1:0)+(g.hPaid?1:0));const isCreator=g.creator===address;const myPaid=isCreator?g.cPaid:g.hPaid;const oppPaid=isCreator?g.hPaid:g.cPaid;const isDone=g.done;const isDraw=g.isDraw;return(<div key={g.id}style={{background:'rgba(0,0,0,0.2)',padding:'0.8rem',borderRadius:'8px',marginBottom:'0.8rem'}}><div style={{display:'flex',justifyContent:'space-between',marginBottom:'0.5rem'}}><span style={{fontWeight:'bold'}}>ID: ...{g.id.slice(-6)}</span><span style={{color:g.status==='playing'?COLORS.accent:COLORS.textSec}}>{g.status}</span></div><div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'0.5rem',fontSize:'0.9rem'}}><div><span style={{color:COLORS.textSec}}>{t('yourStake')}</span><div style={{color:COLORS.accent,fontWeight:'bold'}}>{g.stake?.toLocaleString()||0} GROK {myPaid?'✅':'⏳'}</div></div><div><span style={{color:COLORS.textSec}}>{t('oppStake')}</span><div style={{color:COLORS.accent,fontWeight:'bold'}}>{g.stake?.toLocaleString()||0} GROK {oppPaid?'✅':'⏳'}</div></div></div><div style={{marginTop:'0.5rem',paddingTop:'0.5rem',borderTop:'1px solid rgba(255,255,255,0.1)'}}><span style={{color:COLORS.textSec}}>{t('totalPot')}</span><div style={{color:COLORS.accent,fontSize:'1.2rem',fontWeight:'bold'}}>{totalPot.toLocaleString()} GROK</div><div style={{fontSize:'0.8rem',color:'#60a5fa',marginTop:'0.3rem'}}>{isDone?(isDraw?t('drawRefund'):t('winnerGets')):t('waiting')}</div></div></div>)})}</div>}
...
<button onClick={handleCreateMatch}disabled={loadingTx}style={BtnStyle('#10b981',loadingTx)}>{loadingTx?'⏳ ...':t('cr')}</button>
{inviteLink&&<div style={{marginTop:'0.5rem',padding:'0.8rem',background:'#00332e',borderRadius:'8px',wordBreak:'break-all',fontSize:'0.8rem'}}><div style={{color:COLORS.accent,marginBottom:'4px'}}>{t('invite')}:</div><div style={{color:COLORS.textSec}}>{inviteLink}</div><button onClick={()=>{if(navigator.clipboard)navigator.clipboard.writeText(inviteLink);setMsg(t('cd'))}}style={{...BtnStyle(COLORS.btnBlue,loadingTx),padding:'6px',marginTop:'6px'}}>{cop?t('cd'):t('cp')}</button></div>}
</div>}
...
{gameState==='waiting_funds'&&<p style={{color:'#fbbf24'}}>{t('waiting')} {inviteLink&&<><br/><button onClick={()=>{if(navigator.clipboard)navigator.clipboard.writeText(inviteLink);setMsg(t('cd'))}}style={{...BtnStyle('#3b82f6',loadingTx),marginTop:'8px'}}>{t('cp')}</button></>}</p>}
</div>}
</div>}
