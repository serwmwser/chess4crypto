...
// 🎨 ЦВЕТА ДИЗАЙНА
const COLORS = {
  bg: '#00695c', cardBg: '#004d40', btnBlue: '#1a237e', btnOrange: '#f57c00',
  text: '#ffffff', textSec: '#b2dfdb', accent: '#ffb74d'
}
...
useEffect(()=>{if(!address)return;const loadGames=async()=>{try{const{data,error}=await supabase.from('games').select('*').or(`creator.eq.${address},challenger.eq.${address}`).order('created_at',{ascending:false}).limit(10);if(!error)setActiveGames(data||[])}catch(e){console.warn('Load games failed:',e.message)}};loadGames();const i=setInterval(loadGames,10000);return()=>clearInterval(i)},[address])
...
const handleClaim=(isDraw)=>{if(!gameId){setMsg('⚠️ Game not finished');return}if(!isConnected||!address){setMsg(isDraw?t('drawRefund'):t('winnerGets'));setGameState('idle');return}
  writeContractAsync({address:CHESS_CONTRACT,abi:CHESS_ABI,functionName:'finish',args:[gameId,address,isDraw]}).then(()=>{setMsg(isDraw?t('drawRefund'):t('winnerGets'));setGameState('idle')}).catch(e=>setMsg(e.shortMessage||t('errTx')))}
...
const BtnStyle = (color) => ({
  width: '100%', padding: '12px', background: color, color: '#fff', border: 'none', borderRadius: '10px', fontSize: '1rem', fontWeight: '600', cursor: 'pointer', marginTop: '8px', boxShadow: '0 2px 5px rgba(0,0,0,0.2)'
})
