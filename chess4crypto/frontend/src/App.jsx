...
// ✅ ABI с правильными именами полей (snake_case для Supabase)
...
const handleCreateMatch=async()=>{
  ...
  setLoadingTx(true)
  try{
    // 1. APPROVE: Разрешаем контракту тратить GROK
    setMsg(t('approveTx'))
    ...
    // 2. CREATE: Регистрируем игру в контракте
    setMsg(t('depositTx'))
    ...
    // 3. DEPOSIT: Переводим ставку на контракт
    await writeContractAsync({
      address: CHESS_CONTRACT,
      abi: CHESS_ABI,
    ...
    // 4. Supabase: Сохраняем для отображения всем игрокам
    ...
    if(e.code==='ACTION_REJECTED'||e.message?.includes('rejected')||e.message?.includes('denied')||e.message?.includes('User rejected')){
      ...
      setMsg(t('errTx')+(e.shortMessage||e.message||'Unknown'))
    }
  }finally{
    setLoadingTx(false)
  }
...
const handleJoinMatch=async()=>{
  ...
  try{
    ...
    await writeContractAsync({
      ...
      abi: ERC20_ABI,
      functionName: 'approve',
      args: [CHESS_CONTRACT, parseUnits(pendingJoin.stake.toString(), 18)]
    })
    
    // 2. DEPOSIT: Конвертируем в bytes32
    ...
    // 3. Supabase: Обновляем статус (используем snake_case: hpaid)
    await updateGameStatus(pendingJoin.id,{
      challenger: address,
      hpaid: true,
      status: 'playing',
...
const setupGameSubscription=(id)=>{
  ...
  unsubscribeRef.current=subscribeToGame(id,{
    onGameUpdate:(g)=>{
      // ✅ Используем snake_case: hpaid
      if(g.status==='playing'&&!g.hpaid)setMsg(t('waiting'))
      else if(g.status==='playing'&&g.hpaid)setMsg('♟️ Game started!')
    },
    onMove:(m)=>{
...
const botMove=useCallback(()=>{
  ...
  gameRef.current.move(c);const san=gameRef.current.history({verbose:true}).pop()?.san||`${c.from}${c.to}`;const nf=gameRef.current.fen()
  setFen(nf);setLiveFen(nf);setHist(h=>[...h,nf]);setMoveHistory(mh=>[...mh,{san,from:c.from,to:c.to}])
...
const onDrop=useCallback((src,tgt)=>{
  ...
  try{
    ...
    const san=gameRef.current.history({verbose:true}).pop()?.san||`${src}${tgt}`;const nf=gameRef.current.fen()
    setFen(nf);setLiveFen(nf);setHist(h=>[...h,nf]);setMoveHistory(mh=>[...mh,{san,from:src,to:tgt}])
...
<p style={{color:COLORS.textSec,fontSize:'0.85rem',margin:0,lineHeight:'1.4'}}>{t('guestInfo')}</p>
</div>
...
{/* ⏱️ ВЫБОР ВРЕМЕНИ ИГРЫ В ПРОФИЛЕ (перенесено сюда) */}
