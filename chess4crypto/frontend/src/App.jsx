...
const handleCreateMatch=async()=>{
  ...
  try{
    ...
    setGameId(rawId);setIsDeposited(true);setIsPvP(false) // ✅ Создатель ждёт соперника, бот отключён
...
// 🤝 ПРИСОЕДИНЕНИЕ + ВКЛЮЧЕНИЕ PvP РЕЖИМА
const handleJoinMatch=async()=>{
  ...
  try{
    ...
    setGameState('playing');setIsPvP(true) // ✅ Включаем PvP режим!
...
const botMove=useCallback(()=>{
  ...
  let c;const r=Math.random();if(r<0.7)c=moves[Math.floor(Math.random()*moves.length)];else{const caps=moves.filter(m=>m.captured);c=caps.length?caps[Math.floor(Math.random()*caps.length)]:moves[Math.floor(Math.random()*moves.length)]}
  gameRef.current.move(c);const san=gameRef.current.history({verbose:true}).pop()?.san||`${c.from}${c.to}`;const nf=gameRef.current.fen()
  setFen(nf);setLiveFen(nf);setHist(h=>[...h,nf]);setMoveHistory(mh=>[...mh,{san,from:c.from,to:c.to}]);setMi(i=>i+1);setCurrentMoveIdx(i=>i+1);setIsPlayerTurn(true);setTimerActive('player')
...
const onDrop=useCallback((src,tgt)=>{
  ...
  try{
    ...
    setFen(nf);setLiveFen(nf);setHist(h=>[...h,nf]);setMoveHistory(mh=>[...mh,{san,from:src,to:tgt}]);setMi(i=>i+1);setCurrentMoveIdx(i=>i+1);setIsPlayerTurn(false);setTimerActive('bot');setSelectedSq(null);setPossibleMoves([])
    ...
    if(gameRef.current.isCheckmate()){setGameOver(true);setWinner(address);handleClaim(false)}
      ...
      setMsg(t('bt'))
      // ✅ ЗАПУСКАЕМ БОТА ТОЛЬКО ЕСЛИ НЕ PvP
