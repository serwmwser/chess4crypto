...
const handleSaveProfile=async()=>{if(!address)return;if(profile.bio?.length>500){setMsg('❌ Био макс. 500');return}setProfileLoading(true);try{const ok=await updateProfile(address,profile);if(ok){setMsg('✅ Профиль сохранён!');setIsEditingProfile(false);loadProfile()}else{setMsg('⚠️ Не удалось сохранить профиль')}}catch(e){setMsg('❌ Ошибка: '+e.message)}finally{setProfileLoading(false)}}
