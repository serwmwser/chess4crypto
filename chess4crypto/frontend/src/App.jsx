...
// ✅ Баланс пользователя с авто-рефетчем
...
// ✅ Обновление баланса при изменении данных
useEffect(()=>{
...
// ✅ При подключении: рефетч баланса + загрузка профиля
useEffect(()=>{
  if(isConnected&&address){
    setMsg(t('cn'))
    setView('profile')
    loadProfile()
    // Рефетчим баланс с небольшой задержкой
...
// ✅ Надёжное копирование с фолбэком
...
const handleSaveProfile=async()=>{
  ...
  try{
    ...
    if(e.message?.includes('401')||e.message?.includes('Unauthorized')){
      setMsg('❌ Нет прав. Выполните миграцию RLS в Supabase')
    }else if(e.message?.includes('406')){
      setMsg('❌ Ошибка формата. Проверьте схему таблицы')
    }else{
...
{/* ⏱️ ВЫБОР ВРЕМЕНИ В ГОСТЕВОМ РЕЖИМЕ */}
...
{/* 📖 ИНСТРУКЦИИ ДЛЯ ГОСТЕВОГО РЕЖИМА */}
