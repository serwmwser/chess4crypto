...
export default function App() {
  ...
  // ✅ 4. READ CONTRACTS — БАЛАНС ПОЛЬЗОВАТЕЛЯ И КОНТРАКТА
  const { data: userGrokBalance, refetch: refetchUserBalance, isLoading: balanceLoading } = useReadContract({
    address: GROK_ADDR,
    abi: ERC20_ABI,
    functionName: 'balanceOf',
    args: [address],
    query: {
      enabled: !!address, // ✅ Только если адрес подключён
  ...
  // ✅ 6. ОБНОВЛЕНИЕ БАЛАНСА В СОСТОЯНИИ
  useEffect(() => {
    if (userGrokBalance !== undefined && userGrokBalance !== null) {
      const formatted = Number(formatUnits(userGrokBalance, 18))
      setUserBalance(formatted)
      console.log('💰 Balance updated:', formatted, 'GROK')
    }
  }, [userGrokBalance])
  ...
  const loadAvailableGames = useCallback(async () => { 
    try { 
      const g = await listAvailableGames()
      setAvailableGames(g || []) 
  ...
  const copyToClipboard = async (text) => { 
    ...
    try { 
      ...
      if (ok) { setCop(true); setTimeout(() => setCop(false), 2000); return true } 
    } catch (e) { } 
  ...
  // ✅ СОЗДАНИЕ ИГРЫ С ПРОВЕРКОЙ БАЛАНСА
  const handleCreateMatch = async () => {
    ...
    // ✅ Проверка баланса из состояния (не из hook напрямую)
    if (userBalance < createStake) { 
      setMsg(t('errBal') + ` (нужно: ${createStake}, есть: ${userBalance})`)
      return 
    }
    
    setLoadingTx(true)
    try {
      console.log('🎮 Creating game:', { address, stake: createStake, time: timeCtrl })
      ...
      if (e.message?.includes('rejected') || e.message?.includes('denied')) {
        ...
        setMsg('❌ Контракт отклонил: проверьте approve и gameId')
      } else {
  ...
  const handleJoinMatch = async () => {
    ...
    if (userBalance < pendingJoin.stake) { setMsg(t('errBal')); return }
    ...
    try {
      ...
      setupGameSubscription(pendingJoin.id)
      setMsg(t('successJoin'))
      setPendingJoin(null)
  ...
  const onSqClick = useCallback((sq) => { 
    ...
    if (p && p.color === (isPlayerTurn ? 'w' : 'b')) { 
      setSelectedSq(sq)
      setPossibleMoves(gameRef.current.moves({ square: sq, verbose: true }).map(m => m.to))
      return 
    }
  ...
  const onDrop = useCallback((src, tgt) => { 
    ...
    try { 
      const r = gameRef.current.move({ from: src, to: tgt, promotion: 'q' })
      if (!r) return false
      ...
      if (gameId && address) { 
        setSyncStatus('🔄...')
        recordMove(gameId, address, src, tgt, san, nf).then(() => setSyncStatus('')).catch(() => setSyncStatus('')) 
      }
      if (gameRef.current.isCheckmate()) { setGameOver(true); setWinner(address); handleClaim(false) } 
      else if (gameRef.current.isDraw()) { setGameOver(true); setWinner(null); handleClaim(true) } 
      else { 
  ...
  const sqStyles = useMemo(() => { 
    ...
    possibleMoves.forEach(q => { 
      if (!isReviewMode) s[q] = { backgroundColor: 'rgba(20,85,30,0.5)', backgroundImage: 'radial-gradient(circle,rgba(255,255,255,0.9) 25%,transparent 25%)', backgroundSize: '14px 14px', backgroundPosition: 'center' } 
    })
    return s 
  ...
  const handleSaveProfile = async () => { 
    ...
    try { 
      ...
      if (e.message?.includes('401')) { setMsg('❌ Нет прав. Проверьте RLS в Supabase') } 
  ...
  useEffect(() => { 
    if (isConnected && address) { 
      ...
      setView('profile')
      loadProfile()
      // ✅ Рефетч баланса при подключении
      setTimeout(() => refetchUserBalance?.(), 1000)
    } 
  }, [isConnected, address, t, refetchUserBalance, loadProfile])
  ...
  return (
    <div style={{ minHeight: '100vh', background: COLORS.bg, color: COLORS.text, fontFamily: 'system-ui', padding: '1rem', display: 'flex', justifyContent: 'center', overflowX: 'hidden' }}>
      ...
      {view === 'menu' && <div style={{ maxWidth: '400px', width: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '1rem' }}>
        ...
        {!pendingJoin && <div style={{ background: 'rgba(0,0,0,0.3)', padding: '1rem', borderRadius: '12px', width: '100%', marginBottom: '0.5rem' }}>
          <label style={{ color: COLORS.textSec, display: 'block', marginBottom: '0.3rem' }}>{t('setTime')}</label>
          <select value={timeCtrl} onChange={e => setTimeCtrl(Number(e.target.value))} style={{ width: '100%', padding: '10px', background: COLORS.cardBg, color: '#fff', border: '1px solid rgba(255,255,255,0.2)', borderRadius: '8px', fontSize: '1rem' }}>
            {TIME_OPTIONS.map(m => (<option key={m} value={m}>{m === 1440 ? '24 часа' : `${m} мин`}</option>))}
          </select>
        </div>}
      ...
      {view === 'profile' && <div style={{ maxWidth: '600px', width: '100%', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          ...
          {isEditingProfile ? (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.8rem' }}>
              ...
              <div style={{ display: 'flex', gap: '0.5rem' }}>
                ...
                <button onClick={() => { setIsEditingProfile(false); loadProfile() }} style={BtnStyle('#64748b')}>{t('cl')}</button>
              </div>
            </div>
          ) : (
            <div>
              {profile.avatar && <img src={profile.avatar} alt="av" style={{ width: 80, height: 80, borderRadius: '50%', objectFit: 'cover', margin: '0 auto 1rem', display: 'block', border: '3px solid ' + COLORS.accent }} />}
              <p><b>{t('name')}:</b> {profile.name || '—'}</p>
              <p><b>{t('bio')}:</b> {profile.bio || '—'}</p>
              {profile.website && <p><b>{t('website')}:</b> <a href={profile.website} target="_blank" rel="noopener" style={{ color: '#60a5fa' }}>{profile.website}</a></p>}
              {profile.social && <p><b>{t('social')}:</b> <a href={profile.social.startsWith('http') ? profile.social : '#'} target="_blank" rel="noopener" style={{ color: '#60a5fa' }}>{profile.social}</a></p>}
          ...
          {availableGames.length > 0 ? availableGames.map(g => {
            ...
            return (
              <div key={g.id} style={{ background: 'rgba(0,0,0,0.2)', padding: '0.8rem', borderRadius: '8px', marginBottom: '0.8rem' }}>
                ...
                <div style={{ fontSize: '0.8rem', color: COLORS.textSec, margin: '0.5rem 0' }}>
                  {t('st')}: {g.stake} | ⏱️ {g.time_limit}мин | {t('totalPot')}: {totalPot}GROK
                </div>
                {/* ✅ КНОПКА ПРИСОЕДИНЕНИЯ ДЛЯ ЛЮБОГО ОНЛАЙН-ИГРОКА */}
                {!g.hpaid && g.creator !== address && (
                  <button onClick={() => { setPendingJoin({ id: g.id, stake: g.stake, time: g.time_limit }); handleJoinMatch() }} disabled={loadingTx} style={{ ...BtnStyle('#10b981', loadingTx), marginTop: '0.5rem' }}>
                    {t('jn')} ({g.stake} GROK)
                  </button>
                )}
              </div>
        ...
        {pendingJoin && <div style={{ background: 'linear-gradient(135deg,#1e293b,#334155)', padding: '1.5rem', borderRadius: '12px', textAlign: 'center', border: `2px solid ${COLORS.accent}` }}>
          ...
          <p>{t('needDep')} <b style={{ color: COLORS.accent }}>{pendingJoin.stake.toLocaleString()} GROK</b></p>
          <p style={{ color: COLORS.textSec, fontSize: '0.9rem', marginBottom: '1rem' }}>⏱️ {pendingJoin.time} мин</p>
          <button onClick={handleJoinMatch} disabled={loadingTx} style={BtnStyle('#10b981', loadingTx)}>{`🤝 ${t('jn')}`}</button>
        </div>}

        <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap', justifyContent: 'center' }}>
          {['lobby', 'my', 'create', 'playerProfile'].map(tab => (
            <button key={tab} onClick={() => { setProfileTab(tab); if (tab === 'playerProfile') loadProfile() }} style={{ ...BtnStyle(profileTab === tab ? COLORS.btnBlue : '#334155', loadingTx), flex: '1', minWidth: '80px', padding: '10px 0', fontSize: '0.9rem', marginTop: 0 }}>
              {tab === 'lobby' ? t('availableGames') : tab === 'my' ? t('myG') : tab === 'create' ? t('cr') : t('playerProfile')}
            </button>
          ))}
        ...
        {profileTab === 'create' && <div style={{ background: COLORS.cardBg, padding: '1rem', borderRadius: '12px' }}>
          ...
          <div style={{ marginBottom: '0.8rem' }}>
            <label style={{ color: COLORS.textSec }}>{t('setTime')}</label>
            <select value={timeCtrl} onChange={e => setTimeCtrl(Number(e.target.value))} style={{ width: '100%', padding: '10px', background: '#00332e', color: '#fff', border: '1px solid #00897b', borderRadius: '8px', marginTop: '4px' }}>
              {TIME_OPTIONS.map(m => (<option key={m} value={m}>{m === 1440 ? '24ч' : `${m}м`}</option>))}
            </select>
          </div>
          <div style={{ marginBottom: '0.5rem' }}>
            ...
            <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
              {STAKE_OPTIONS.map(a => (
                <button key={a} onClick={() => setCreateStake(a)} style={{ ...BtnStyle(createStake === a ? '#10b981' : '#334155', loadingTx), flex: '1', minWidth: '70px', padding: '10px 0', fontSize: '0.9rem', marginTop: 4 }}>
                  {a.toLocaleString()}
                </button>
              ))}
            </div>
          ...
          <button onClick={handleCreateMatch} disabled={loadingTx} style={BtnStyle('#10b981', loadingTx)}>
            {loadingTx ? (txHash ? t('confirmingTx') : t('approveTx')) : t('cr')}
          </button>
          {inviteLink && <div style={{ marginTop: '1rem', padding: '0.8rem', background: '#00332e', border: `1px solid ${COLORS.accent}`, borderRadius: '8px', wordBreak: 'break-all', fontSize: '0.8rem' }}>
            <div style={{ color: COLORS.accent, marginBottom: '4px', fontWeight: 'bold' }}>{t('invite')}:</div>
            <div style={{ color: COLORS.textSec, marginBottom: '8px', cursor: 'pointer' }} onClick={() => copyToClipboard(inviteLink)}>
              {inviteLink}<br />
              <small style={{ color: COLORS.textSec }}>{cop ? t('cd') : t('clickToCopy')}</small>
            </div>
          </div>}
        </div>}
        ...
        {profileTab === 'my' && <div style={{ background: COLORS.cardBg, padding: '1rem', borderRadius: '12px' }}>
          ...
          {contractBalance !== null && <p style={{ color: COLORS.textSec, fontSize: '0.8rem' }}>🏦 {t('pot')} {contractBalance.toLocaleString()} GROK</p>}
          ...
          {gameState === 'waiting_funds' && <div style={{ background: 'rgba(0,0,0,0.3)', padding: '0.8rem', borderRadius: '8px', marginTop: '0.5rem' }}>
            <p style={{ color: '#fbbf24', margin: 0 }}>{t('waiting')} ⏱️ {timeCtrl} мин</p>
      ...
      {view === 'game' && <div style={{ maxWidth: '100%', width: '100%', margin: '0 auto', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
        <div style={{ display: 'flex', justifyContent: 'space-around', width: '100%', maxWidth: '420px', marginBottom: '0.5rem' }}>
          <div style={{ background: '#004d40', padding: '10px 20px', borderRadius: '10px', color: '#fff', textAlign: 'center', border: '1px solid #00897b', width: '45%' }}>
            <div style={{ fontSize: '0.85rem', opacity: 0.8 }}>{t('y')}</div>
            <div style={{ fontSize: '1.6rem', fontWeight: 'bold', fontFamily: 'monospace' }}>{fmtTime(pTime)}</div>
          </div>
          <div style={{ background: '#004d40', padding: '10px 20px', borderRadius: '10px', color: '#fff', textAlign: 'center', border: '1px solid #00897b', width: '45%' }}>
            <div style={{ fontSize: '0.85rem', opacity: 0.8 }}>{t('b')}</div>
            <div style={{ fontSize: '1.6rem', fontWeight: 'bold', fontFamily: 'monospace' }}>{fmtTime(bTime)}</div>
          </div>
        </div>
        ...
        {moveHistory.length > 0 && <div style={{ display: 'flex', gap: '0.5rem', marginTop: '0.5rem', justifyContent: 'center' }}>
          <button onClick={prevMove} disabled={currentMoveIdx <= 0} style={{ ...BtnStyle('#334155', loadingTx), width: 'auto', padding: '8px 16px' }}>{t('prev')}</button>
          <button onClick={nextMove} disabled={currentMoveIdx >= hist.length - 1} style={{ ...BtnStyle('#334155', loadingTx), width: 'auto', padding: '8px 16px' }}>{t('next')}</button>
          {isReviewMode && <button onClick={resumeLive} disabled={loadingTx} style={{ ...BtnStyle('#10b981', loadingTx), width: 'auto', padding: '8px 16px' }}>{t('live')}</button>}
        </div>}
        <div style={{ display: 'flex', gap: '0.5rem', marginTop: '0.5rem', justifyContent: 'center' }}>
          <button onClick={() => { resetGame(); setView('profile') }} disabled={loadingTx} style={{ ...BtnStyle(COLORS.btnBlue, loadingTx), width: 'auto', flex: '1' }}>{t('newG')}</button>
          <button onClick={() => { resetGame(); startGame() }} disabled={loadingTx} style={{ ...BtnStyle('#10b981', loadingTx), width: 'auto', flex: '1' }}>{t('botG')}</button>
        </div>
        {moveHistory.length > 0 && <div style={{ marginTop: '0.5rem', background: COLORS.cardBg, padding: '0.5rem', borderRadius: '8px', maxHeight: '100px', overflowY: 'auto', fontSize: '0.8rem' }}>
          {moveHistory.map((m, i) => (<span key={i} style={{ marginRight: '0.5rem', background: currentMoveIdx === i ? COLORS.btnBlue : 'rgba(0,0,0,0.3)', padding: '0.2rem 0.4rem', borderRadius: '4px' }}>{i + 1}.{m.san}</span>))}
        </div>}
        {gameOver && <div style={{ marginTop: '1.5rem', background: '#1e293b', padding: '1.5rem', borderRadius: '12px', textAlign: 'center', border: `2px solid ${winner === address ? '#10b981' : '#ef4444'}`, maxWidth: '400px', width: '100%' }}>
          ...
          {winner === address && gameId && <button onClick={() => handleClaim(false)} disabled={loadingTx} style={{ ...BtnStyle('#10b981', loadingTx), fontSize: '1.2rem', marginBottom: '1rem' }}>{loadingTx ? '⏳...' : t('claimBtn')}</button>}
          <button onClick={() => { resetGame(); setView('profile') }} style={BtnStyle(COLORS.btnBlue)}>{t('newG')}</button>
        </div>}
      </div>}
        ...
        <div style={{ background: COLORS.cardBg, padding: '1.5rem', borderRadius: '16px', maxWidth: '360px' }} onClick={e => e.stopPropagation()}>
          ...
          <ol style={{ color: COLORS.textSec, margin: '1rem 0', paddingLeft: '1.2rem' }}>
            ...
            <li>{t('g3')}<br /><code style={{ background: '#00332e', padding: '0.3rem', borderRadius: '4px', display: 'block', margin: '0.3rem 0', fontSize: '0.8rem' }}>{GROK_ADDR}</code>
              <button onClick={copyAddr} style={{ ...BtnStyle(COLORS.btnBlue, loadingTx), marginTop: '0.5rem', padding: '8px' }}>{cop ? t('cd') : t('cp')}</button>
            </li>
          </ol>
