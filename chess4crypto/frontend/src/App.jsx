...
// 📏 Исправление для Android (убирает смещение)
useEffect(()=>{
...
<div>{t('st')}: {g.stake}</div><div>{t('tc')} {g.time_limit} мин</div><div>{isDone?(isDraw?t('drawRefund'):t('winnerGets')):t('waiting')}</div>
</div>
...
{pendingJoin&&<div style={{background:'linear-gradient(135deg,#1e293b,#334155)',padding:'1.5rem',borderRadius:'12px',textAlign:'center',border:`2px solid ${COLORS.accent}`}}><h3 style={{color:COLORS.accent,margin:'0 0 0.5rem'}}>{t('invite')}</h3><p style={{fontSize:'1.1rem'}}>{t('needDep')} <strong style={{color:COLORS.accent}}>{pendingJoin.stake.toLocaleString()} GROK</strong></p><p style={{color:COLORS.textSec,fontSize:'0.9rem',marginBottom:'1rem'}}>{t('tc')}: {pendingJoin.time} мин</p><button onClick={handleJoinMatch}disabled={loadingTx}style={BtnStyle('#10b981',loadingTx)}>{`🤝 ${t('jn')}`}</button></div>}
...
{/* ⏱️ Отображение выбранного времени */}
...
<div style={{background:COLORS.cardBg,padding:'8px',borderRadius:'12px',display:'flex',justifyContent:'center'}}>
...
{gameOver&&<div style={{marginTop:'1.5rem',background:'#1e293b',padding:'1.5rem',borderRadius:'12px',textAlign:'center',border:`2px solid ${winner===address?'#10b981':'#ef4444'}`}}>
  <h2 style={{color:COLORS.accent,margin:'0 0 1rem'}}>{winner===address?t('victoryTitle'):winner==='bot'?t('defeatTitle'):t('drawTitle')}</h2>
  {/* 💰 Кнопка выигрыша только для реальных игр (когда есть gameId) */}
